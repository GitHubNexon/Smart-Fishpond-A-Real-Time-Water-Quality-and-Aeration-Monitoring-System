...
import RepairFormModal from '@/modals/repair-form-modals/repair-form';

export function useRepairTableLogic() {
  ...
  useEffect(() => {
    const checkHash = () => {
      if (
        window.location.hash === '#add-repair-transaction' ||
        window.location.hash === '#edit-repair-transaction'
      ) {
        setModalOpen(true);
      } else {
        setModalOpen(false);
  ...
  const fetchAssetTransaction = async (params: {
    ...
    try {
      const hardcodedFilter = {
        transactionType: [
          'request_repair', // Available -> ForRepair
          'return_for_repair', // Issued -> ForRepair
          'send_to_repair', // ForRepair -> Repaired
          'complete_repair', // Repiared -> Available
          'fail_repair', // Repaired -> RepairFailed
        ],
      };
  ...
  const handleAddRepairTransaction = () => {
    window.location.hash = 'add-repair-transaction';
