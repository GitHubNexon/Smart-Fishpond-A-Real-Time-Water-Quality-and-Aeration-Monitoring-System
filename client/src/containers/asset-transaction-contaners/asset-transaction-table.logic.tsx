...
export function useAssetTransactionTableLogic() {
  ...
  useEffect(() => {
    const checkHash = () => {
      if (
        window.location.hash === '#add-asset-transaction' ||
        window.location.hash === '#edit-asset-transaction'
      ) {
        setModalOpen(true);
      } else {
        setModalOpen(false);
  ...
  const fetchAssetTransaction = async (params: {
    ...
    try {
      // const hardcodedFilter = { transactionType: 'request_transfer' };
  ...
  const handleEditAsset = (transaction: AssetTransactionInfo) => {
    window.location.hash = 'edit-asset-transaction';
