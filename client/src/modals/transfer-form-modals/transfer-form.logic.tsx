...
interface TransactionData {
  ...
  custodian?: any; // Optional - some transactions don't have custodians
...
const AUTO_POPULATE_CONFIG = {
  [TransferTransactionType.APPROVE_TRANSFER]: {
    needsTransactionNo: true,
    statusFilter: [InventoryStatus.ForTransfer],
  },
  [TransferTransactionType.REJECT_TRANSFER]: {
    needsTransactionNo: true,
    statusFilter: [InventoryStatus.ForTransfer],
  },
  [TransferTransactionType.MAKE_AVAILABLE]: {
    needsTransactionNo: true,
    statusFilter: [
      InventoryStatus.ReturnedToCustodian,
      InventoryStatus.Transferred,
    ],
  },
};

export default function useTransferFormLogic(
  open: boolean,
  close: () => void,
  onSuccess?: () => void,
) {
  ...
  const [formData, setFormData] = useState<TransferFormData>({
    inventoryIds: [],
    transactionType: TransferTransactionType.REQUEST_TRANSFER,
    custodianId: '',
  ...
  const autoPopulateFromTransaction = async (transactionNo: string) => {
    ...
    try {
      ...
      if (
        custodian &&
        formData.transactionType !== TransferTransactionType.MAKE_AVAILABLE
      ) {
      ...
      setFormData((prev) => ({
        ...
        transferFromLocation: transactions[0].transferFromLocation || '',
        transferToLocation: transactions[0].transferToLocation || '',
        transferFromDepartment: transactions[0].transferFromDepartment || '',
        transferToDepartment: transactions[0].transferToDepartment || '',
      }));
  ...
  const fetchAvailableInventory = async (
    ...
    try {
      ...
      switch (transactionType) {
        case TransferTransactionType.REQUEST_TRANSFER:
          statuses = [InventoryStatus.Available, InventoryStatus.NewAvailable];
          break;
        case TransferTransactionType.APPROVE_TRANSFER:
          statuses = [InventoryStatus.ForTransfer];
          break;
        case TransferTransactionType.REJECT_TRANSFER:
          statuses = [InventoryStatus.ForTransfer];
          break;
        case TransferTransactionType.MAKE_AVAILABLE:
          statuses = [
            InventoryStatus.ReturnedToCustodian,
            InventoryStatus.Transferred,
          ];
  ...
  const handleTransactionTypeSelect = (type: TransferTransactionType) => {
    setFormData({
      ...defaultFormData,
      transactionType: type,
      ...(type === TransferTransactionType.REQUEST_TRANSFER && {
        approvedAt: formatDate.dateTime(new Date()),
      }),
      ...(type === TransferTransactionType.REJECT_TRANSFER && {
        rejectedAt: formatDate.dateTime(new Date()),
      }),
      ...(type === TransferTransactionType.APPROVE_TRANSFER && {
        approvedAt: formatDate.dateTime(new Date()),
      }),
      ...(type === TransferTransactionType.MAKE_AVAILABLE && {
        custodianId: undefined,
      }),
    });

    if (type === TransferTransactionType.MAKE_AVAILABLE) {
  ...
  const handleSubmitTransaction = async (e?: React.FormEvent | boolean) => {
    ...
    try {
      ...
      if (formData.transferFromLocation)
        payload.transferFromLocation = formData.transferFromLocation;
      if (formData.transferToLocation)
        payload.transferToLocation = formData.transferToLocation;
      if (formData.transferFromDepartment)
        payload.transferFromDepartment = formData.transferFromDepartment;
      if (formData.transferToDepartment)
        payload.transferToDepartment = formData.transferToDepartment;

      if (
        formData.transactionType === TransferTransactionType.APPROVE_TRANSFER
      ) {
        if (formData.approvedAt) payload.approvedAt = formData.approvedAt;
      }

      if (
        formData.transactionType === TransferTransactionType.REJECT_TRANSFER
      ) {
        if (formData.rejectedAt) payload.rejectedAt = formData.rejectedAt;
  ...
  const isFormValid = useMemo(() => {
    ...
    return (
      ...
      (!needsCustodian || formData.custodianId !== '') &&
