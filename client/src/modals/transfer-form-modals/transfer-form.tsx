...
import useTransferFormLogic from './transfer-form.logic';
...
import { TransferTransactionType } from './transfer-form.interface';
import { Checkbox } from '@/components/ui/checkbox';
import TransactionNoPopUp from '@/modals/components/transaction-no-pop-up';

const TransferFormModal: React.FC<FormModalProps> = ({
  open,
  close,
  onSuccess,
}) => {
  const {
    ...
    filteredInventory,
  } = useTransferFormLogic(open, close, onSuccess);

  const transactionTypeOptions = [
    {
      type: TransferTransactionType.REQUEST_TRANSFER,
      label: 'Request Transfer',
      description: 'Request items for Transfer',
    ...
    },
    {
      ...
      description: 'Approve pending requests for transfer',
    ...
    },
    {
      ...
      description: 'Reject pending requests for transfer',
      icon: <XCircle className="w-6 h-6 text-red-500" />,
      color:
        'border-red-200 dark:border-red-800 hover:bg-red-50 dark:hover:bg-red-950',
    },
    {
      type: TransferTransactionType.MAKE_AVAILABLE, // NEW
                ...
                <>
                  ...
                  <Card className="border-2 border-primary/20">
                    ...
                    <CardContent className="p-6 space-y-4">
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="transferFromLocation">
                          Transfer From Location (Optional)
                        </Label>
                        <Textarea
                          id="transferFromLocation"
                          placeholder="Origin of the transfer (e.g., Main Warehouse)"
                          value={formData.transferFromLocation || ''}
                          onChange={(e) =>
                            setFormData({
                              ...formData,
                              transferFromLocation: e.target.value,
                            })
                          }
                        />
                      </div>
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="transferToLocation">
                          Transfer To Location (Optional)
                        </Label>
                        <Textarea
                          id="transferToLocation"
                          placeholder="Destination of the transfer"
                          value={formData.transferToLocation || ''}
                          onChange={(e) =>
                            setFormData({
                              ...formData,
                              transferToLocation: e.target.value,
                            })
                          }
                        />
                      </div>
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="transferFromDepartment">
                          Transfer from Department (Optional)
                        </Label>
                        <Textarea
                          id="transferFromDepartment"
                          placeholder="Originating department (e.g., HR, IT)"
                          value={formData.transferFromDepartment || ''}
                          onChange={(e) =>
                            setFormData({
                              ...formData,
                              transferFromDepartment: e.target.value,
                            })
                          }
                        />
                      </div>
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="transferToDepartment">
                          Transfer To Department (Optional)
                        </Label>
                        <Textarea
                          id="transferToDepartment"
                          placeholder="Destination department (e.g., HR, IT)"
                          value={formData.transferToDepartment || ''}
                          onChange={(e) =>
                            setFormData({
                              ...formData,
                              transferToDepartment: e.target.value,
                            })
                          }
                        />
                      </div>
        ...
        <ScrollArea className="h-full">
          <div className="space-y-4">
            ...
            <div className="bg-primary/5 border border-primary/20 rounded-lg p-4">
              ...
              {formData.transferFromDepartment && (
                <PreviewField
                  label="Transfer From Department"
                  value={formData.transferFromDepartment}
                />
              )}
              {formData.transferToDepartment && (
                <PreviewField
