...
const AUTO_POPULATE_CONFIG = {
  [IssuanceTransactionType.APPROVE_ISSUANCE]: {
    ...
    statusFilter: [InventoryStatus.ForIssuance],
  },
  [IssuanceTransactionType.REJECT_ISSUANCE]: {
    needsTransactionNo: true,
    statusFilter: [InventoryStatus.ForIssuance],
  },
  [IssuanceTransactionType.RETURN_TO_INVENTORY]: {
    needsTransactionNo: true,
    statusFilter: [InventoryStatus.Issued],
  },
  [IssuanceTransactionType.MAKE_AVAILABLE]: {
    needsTransactionNo: true,
    statusFilter: [
...
export default function useIssuanceFormLogic(
  open: boolean,
  close: () => void,
  onSuccess?: () => void,
) {
  ...
  const [formData, setFormData] = useState<IssuanceFormData>({
    inventoryIds: [],
    transactionType: IssuanceTransactionType.DIRECT_ISSUANCE,
    custodianId: '',
  ...
  const defaultFormData: IssuanceFormData = {
    inventoryIds: [],
    transactionType: IssuanceTransactionType.DIRECT_ISSUANCE,
    custodianId: '',
    transactionDate: formatDate.dateTime(new Date()),
  ...
  const autoPopulateFromTransaction = async (transactionNo: string) => {
    ...
    try {
      ...
      // Only set custodian if it exists
      if (custodian) {
  ...
  const fetchAvailableInventory = async (
    ...
    try {
      ...
      switch (transactionType) {
        case IssuanceTransactionType.DIRECT_ISSUANCE:
        case IssuanceTransactionType.REQUEST_ISSUANCE:
          statuses = [InventoryStatus.Available, InventoryStatus.NewAvailable];
          break;
        case IssuanceTransactionType.APPROVE_ISSUANCE:
        case IssuanceTransactionType.REJECT_ISSUANCE:
          statuses = [InventoryStatus.ForIssuance];
          break;
        case IssuanceTransactionType.RETURN_TO_INVENTORY:
          statuses = [InventoryStatus.Issued];
          break;
        case IssuanceTransactionType.MAKE_AVAILABLE:
          statuses = [
            InventoryStatus.ReturnedToCustodian,
            InventoryStatus.Transferred,
          ];
  ...
  const handleTransactionTypeSelect = (type: IssuanceTransactionType) => {
    setFormData({
      ...defaultFormData,
  ...
  const handleSubmitTransaction = async (e?: React.FormEvent | boolean) => {
    ...
    try {
      ...
      if (
        formData.transactionType === IssuanceTransactionType.DIRECT_ISSUANCE ||
        formData.transactionType === IssuanceTransactionType.APPROVE_ISSUANCE
      ) {
        if (formData.approvedAt) payload.approvedAt = formData.approvedAt;
      }

      if (
        formData.transactionType === IssuanceTransactionType.REJECT_ISSUANCE
      ) {
