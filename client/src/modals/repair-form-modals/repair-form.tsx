...
import { RepairTransactionType } from './repair-form.interface';
import { Checkbox } from '@/components/ui/checkbox';
import TransactionNoPopUp from '../components/transaction-no-pop-up';
import useRepairFormLogic from './repair-form.logic';
import { formatNumber } from '@/lib/format-number.util';
import { useFormattedInput } from '@/hooks/useFormattedInput';

const RepairFormModal: React.FC<FormModalProps> = ({
  open,
  close,
  onSuccess,
}) => {
  const {
    ...
    filteredInventory,
  } = useRepairFormLogic(open, close, onSuccess);

  const handleNumericChange = (name: string, value: number) => {
    setFormData((prev) => ({ ...prev, [name]: value }));
  };

  const repairCostInput = useFormattedInput({
    name: 'repairCost',
  ...
  const transactionTypeOptions = [
    {
      type: RepairTransactionType.REQUEST_REPAIR,
      label: 'Request Repair',
      description: 'Submit a request to repair an item.',
      icon: <FileText className="w-6 h-6 text-blue-500" />,
      color:
        'border-blue-200 dark:border-blue-800 hover:bg-blue-50 dark:hover:bg-blue-950',
    },
    {
      type: RepairTransactionType.SEND_TO_REPAIR,
      label: 'Send for Repair',
      description: 'Send the item to the repair facility for processing.',
      icon: <Wrench className="w-6 h-6 text-green-500" />, // better for "repair"
      color:
        'border-green-200 dark:border-green-800 hover:bg-green-50 dark:hover:bg-green-950',
    },
    {
      type: RepairTransactionType.COMPLETE_REPAIR,
      label: 'Repair Completed',
      description: 'The repair has been successfully completed.',
      icon: <CheckCircle2 className="w-6 h-6 text-emerald-500" />,
      color:
        'border-emerald-200 dark:border-emerald-800 hover:bg-emerald-50 dark:hover:bg-emerald-950',
    },
    {
      type: RepairTransactionType.FAIL_REPAIR,
      label: 'Repair Failed',
      description: 'The repair attempt was unsuccessful.',
      icon: <XCircle className="w-6 h-6 text-red-500" />,
      color:
        'border-red-200 dark:border-red-800 hover:bg-red-50 dark:hover:bg-red-950',
    },
    {
      type: RepairTransactionType.RETURN_FOR_REPAIR,
      label: 'Return for Repair',
      description: 'Return the currently issued item for repair.',
      icon: <RotateCcw className="w-6 h-6 text-purple-500" />, // better icon for "return"
      color:
  ...
  return (
    <>
                  ...
                  <Card className="border-2 border-primary/20">
                    ...
                    <CardContent className="p-6 space-y-4">
                      ...
                      {/* {(formData.transactionType ===
                        RepairTransactionType.DIRECT_ISSUANCE ||
                        formData.transactionType ===
                          RepairTransactionType.APPROVE_ISSUANCE) && (
                        <div className="space-y-2">
                          <DatePicker
                            label="Approved Date"
                      ...
                      {/* {formData.transactionType ===
                        RepairTransactionType.REJECT_ISSUANCE && (
                        <>
                          <div className="space-y-2">
                            <DatePicker
                              label="Rejected Date"
                            ...
                            >
                              ...
                              <div className="flex-1">
                                ...
                                <p className="text-sm text-muted-foreground">
                                  Custodian:{' '}
                                  {item.custodian
                                    ? `${item.custodian.firstName || 'N/A'} ${
                                        item.custodian.middleName || ''
                                      } ${item.custodian.lastName || 'N/A'}`
                                    : 'N/A'}
                                </p>
                              </div>
                            </div>
      ...
      <FormPreviewer
        ...
        headerTitle="Review Repair Transaction Before Submitting"
        description="Please review all repair transaction details before confirming."
        before={[]}
        after={[]}
          ...
          <div className="space-y-4">
            ...
            <div className="bg-primary/5 border border-primary/20 rounded-lg p-4">
              <h3 className="font-semibold text-foreground mb-3 flex items-center gap-2">
                ...
                Repair Transaction Information
              </h3>
              <PreviewField
              ...
              {formData.repairCost !== undefined && (
                <PreviewField
                  label="Repair Cost"
                  value={formatNumber.currency(
                    formData.repairCost.valueOf(),
                    'PHP',
                  )}
                />
              )}
